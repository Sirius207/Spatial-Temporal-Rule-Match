...
def on_message(client, userdata, msg):
    ...
    def CalMccCenter(mcc):
        ...
        if len(mcc) == 2:
            center = list()
            center.append((EventDict[mcc[0]]["Lon"] +
                           EventDict[mcc[1]]["Lon"]) / 2)
            center.append((EventDict[mcc[0]]["Lat"] +
                           EventDict[mcc[1]]["Lat"]) / 2)
            #print (center)
            return center
        else:
    ...
    def CalMccCenterByThreePoints(mcc):
        ...
        center = list()
        ma = (EventDict[mcc[1]]["Lat"] - EventDict[mcc[0]]["Lat"]) / \
            (EventDict[mcc[1]]["Lon"] - EventDict[mcc[0]]["Lon"])
        mb = (EventDict[mcc[2]]["Lat"] - EventDict[mcc[1]]["Lat"]) / \
            (EventDict[mcc[2]]["Lon"] - EventDict[mcc[1]]["Lon"])
    ...
    if msg.topic == 'ncku/netdb/test':
        ...
        for x in EventDict:
            ...
            DistanceTmp = distance(
                Lon, Lat, EventDict[x]["Lon"], EventDict[x]["Lat"])
